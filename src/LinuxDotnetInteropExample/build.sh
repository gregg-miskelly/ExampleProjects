...
{
    echo "Install the .NET SDK, clang and cmake to build"
}
...
check_prereqs()
{
    ...
    hash dotnet 2>/dev/null || { echo >&2 "Please install the .NET SDK before running this script"; exit 1; }
}
...
locate_build_tools()
{
    ...
    if [[ $__CrossBuild == 1 && "${OSName}" != "Darwin"  ]]; then
        ...
        if [[ -z "$CONFIG_DIR" ]]; then
            CONFIG_DIR="$SlnRoot/cross/$__BuildArch"
        fi
...
invoke_build()
{
    ...
    if ! dotnet build -p OutputPath=$__BinDir; then
    ...
    # cmake_args+=" -DREPOSITORY_ROOT=$SlnRoot"
    ...
    cmake_args+=" $SlnRoot/MyNativeLib"
...
# Set the root of the solution
pushd $script_dir > /dev/null
SlnRoot=$(pwd)
popd > /dev/null
if [ ! -f "$SlnRoot/build.sh" ]; then
...
OSOutRoot=$SlnRoot/out/$__BuildOS
